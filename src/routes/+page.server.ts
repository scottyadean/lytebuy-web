...
export const load: PageServerLoad = async ({ fetch, setHeaders }) => {
	// Fetch the posts, the vendor count and the featured vendor together; none
	// blocks the others, and all degrade to an empty state on failure rather
	// than throwing.
	const [{ items }, vendorCount, featuredVendor] = await Promise.all([
	...
	return { posts: items, vendorCount, featuredVendor };
};
