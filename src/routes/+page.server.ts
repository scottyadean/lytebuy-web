...
import { getDirectoryVendors, getFeaturedVendor, getVendorCount } from '$lib/server/vendors';
...
export const load: PageServerLoad = async ({ fetch, setHeaders }) => {
	...
	const [{ items }, vendorCount, featuredVendor, products, vendors] = await Promise.all([
		listContent(fetch, 'post', { limit: 3 }),
		getVendorCount(fetch),
		getFeaturedVendor(fetch),
		getFeedProducts(fetch),
		getDirectoryVendors(fetch)
	]);
	...
	return { posts: items, vendorCount, featuredVendor, products, vendors };
};
